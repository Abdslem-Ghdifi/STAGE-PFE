...
const MessagerieFormateur = () => {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://localhost:8080/api/contact/msg', {
          headers: {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:8080/api/contact/envoyerFormateur',
        newMessage,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const openMessageDetail = (message) => {
    ...
    if (!message.isRead && message.recipient.role === 'formateur') {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="container mx-auto px-4 py-8 flex-grow">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-bold text-gray-800">Messagerie Formateur</h1>
          <button
          ...
          {messages.length === 0 ? (
            ...
            <div className="divide-y divide-gray-200">
              {messages.map((message) => (
                <div 
                  key={message._id} 
                  className={`p-4 hover:bg-gray-50 cursor-pointer ${!message.isRead && message.recipient.role === 'formateur' ? 'bg-blue-50' : ''}`}
                  onClick={() => openMessageDetail(message)}
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-medium truncate ${!message.isRead && message.recipient.role === 'formateur' ? 'text-blue-600' : 'text-gray-900'}`}>
                        {message.sender.role === 'formateur' ? 'Moi' : 'Administrateur'}
                      </p>
                    ...
                    <div className="ml-4 flex-shrink-0 flex">
                      ...
                      {!message.isRead && message.recipient.role === 'formateur' && (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6">
            ...
            <div className="mb-6">
              <div className="flex justify-between text-sm text-gray-600 mb-2">
                <div>
                  ...
                  {selectedMessage.sender.role === 'formateur' ? 'Moi' : 'Administrateur'}
                </div>
              ...
              <div className="text-sm text-gray-600 mb-4">
                ...
                {selectedMessage.recipient.role === 'formateur' ? 'Moi' : 'Administrateur'}
              </div>
            ...
            <div className="flex justify-end">
              {selectedMessage.sender.role !== 'formateur' && (
                <button
...
export default MessagerieFormateur;